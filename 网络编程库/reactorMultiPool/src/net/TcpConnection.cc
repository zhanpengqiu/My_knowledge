    ...
    , highWaterMark_(64 * 1024 * 1024) // 64M 避免发送太快对方接受太慢
{
    ...
    std::cout << "TcpConnection::ctor[" << name_.c_str() << "] at fd =" << sockfd;
...
void TcpConnection::sendInLoop(const void* data, size_t len)
{
    ...
    {
        std::cerr  << "disconnected, give up writing";
        return;
    }
...
void TcpConnection::handleRead(Timestamp receiveTime)
{
    ...
    else
    {
        // 出错情况
        errno = savedErrno;
        std::cerr  << "TcpConnection::handleRead() failed";
        handleError();
    }
}
